...
def check_cfg():
	...
	if(not file_exists):
		...
		config['DEFAULT'] = {'Grabar' : 'no', 'Fecha': '01/09/2024', 'NombreArchivo': 'prueba_192Ks_32bits'}
		config['sound.format'] = {'Muestreo': '192000', 'Bits': '32'}
		config['record.time'] = {'TiempoTotal': '10', 'TiempoArchivo': '10', 'Delay': '0', 'Index': '0'}
		with open(path_to_file+cfg_file, 'w') as configfile:
			config.write(configfile)


def check_grabar(config):
	rt = config['DEFAULT']
	if (rt['Grabar'] == 'no'):

		#ejectar un comando del sistema operativo
		# parar rc.local
...
def print_cfg(config):
	print (config['DEFAULT']['Fecha'])
	print (config['DEFAULT']['NombreArchivo'])
	...
	rt = config['record.time']
	print(rt['TiempoTotal'])
	print(rt['TiempoArchivo'])
	print(rt['Delay'])
	print(rt['Index'])
...
def rec_cmd(config):
	...
	tm = time.strptime("{:s}".format(config['DEFAULT']['Fecha']), "%d/%m/%Y")
	...
	cmd = 'arecord -D hw:0,0 -d {:s} --max-file-time {:s} -c 2 -r {:s} -f S32_LE -t wav --use-strftime {:s}/%Y/%m/%d/{:s}_{:s}-%v.wav' \
		.format(rt['TiempoTotal'],rt['TiempoArchivo'],sf['Muestreo'],path_to_file,config['DEFAULT']['NombreArchivo'],rt['Index'])
	#print(cmd)
	return cmd
