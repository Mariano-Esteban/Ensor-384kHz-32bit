...
•	Four input to balanced and not balanced microphones, or line in.
...
The Ensor-4_Channels card is only compatible with Raspberry Pi 5 for recording 4 channesls simultaneously.  

Ensor-4_Channels is a professional audio digitizer card capable of digitizing two audio channels with a signal sampling rate of up to 384,000 samples per second and a resolution of 32 bits per sample. The digitized signal is sent to the Raspberry Pi via I2S for recording on a USB flash drive

The Ensor-4_Channels card has four input for connecting external differential microphones, single ended microphones or line in

The Ensor-4_Channels card connects to the Raspberry Pi via the 40-pin GPIO expansion connector
...
The Ensor-4_Channels board design is based on the Texas Instruments evaluation board
...
The recommended differential microphone to use with the card is the Infineon IM73A135V01
Reference:
